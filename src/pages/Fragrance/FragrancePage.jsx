    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const response = await getProducts(currentPage, 3, filterParams)
        ...
        <div className="min-h-screen bg-white">
            <div className="max-w-screen-xl mx-auto px-4 md:px-6 py-6">
                ...
                <div className="flex gap-6">
                    <aside className="hidden md:block flex-shrink-0 overflow-hidden"
                        ...
                        <FilterSidebar category="fragrance" selected={selectedFilters} onToggle={toggleFilter} onClear={clearFilters} onClose={() => setFilterOpen(false)} />
                    </aside>
            ...
            <div className={`md:hidden fixed inset-x-0 bottom-0 z-50 bg-white rounded-t-2xl transition-transform duration-300 ease-out ${filterOpen ? 'translate-y-0' : 'translate-y-full'}`}
                ...
                <div className="p-5 h-full overflow-y-auto">
                    ...
                    <FilterSidebar category="fragrance" selected={selectedFilters} onToggle={toggleFilter} onClear={clearFilters} onClose={() => setFilterOpen(false)} isMobile />
                </div>
            </div>
        </div>
    )
